...
CFLAGS = -g -fprofile-arcs -ftest-coverage -Werror-implicit-function-declaration -g -O0 -Wno-system-headers -Wenum-conversion -pthread
CFLAG_CURL_SYMBOL_HIDING = -fvisibility=hidden
CONFIGURE_OPTIONS = " '--with-openssl' '--disable-shared' '--enable-silent-rules' '--enable-debug' '--disable-optimize' '--disable-warnings' '--disable-curldebug' '--disable-symbol-hiding' '--disable-hidden-symbols' '--enable-dependency-tracking' '--disable-file' '--disable-ldap' '--enable-ldaps' '--enable-proxy' '--enable-dict' '--disable-telnet' '--enable-tftp' '--enable-pop3' '--disable-imap' '--enable-smtp' '--enable-gopher' '--enable-manual' '--enable-libcurl-option' '--enable-ipv6' '--enable-versioned-symbols' '--disable-verbose' '--enable-sspi' '--disable-crypto-auth' '--disable-ntlm-wb' '--enable-tls-srp' '--enable-unix-sockets' '--disable-socketpair' '--enable-http-auth' '--enable-doh' '--enable-mime' '--disable-dateparse' '--disable-progress-meter' '--enable-get-easy-options' '--enable-hidden-symbols' '--enable-code-coverage' '--enable-static' '--enable-fast-install'"
CPP = gcc -E
CPPFLAGS = -DNDEBUG
...
LDFLAGS = 
LIBCURL_LIBS = -lssl -lcrypto -lssl -lcrypto -lzstd -lz -lgcov -pthread
LIBCURL_NO_SHARED = -lssl -lcrypto -lssl -lcrypto  -lzstd  -lz  -lgcov -pthread
LIBOBJS = 
LIBS = -lssl -lcrypto -lssl -lcrypto -lzstd -lz -lgcov
...
STRIP = strip
SUPPORT_FEATURES = AsynchDNS HSTS HTTPS-proxy IPv6 Largefile SSL TLS-SRP UnixSockets alt-svc libz zstd
SUPPORT_PROTOCOLS = DICT FTP FTPS GOPHER GOPHERS HTTP HTTPS MQTT POP3 POP3S RTSP SMTP SMTPS TFTP
USE_ARES = 
USE_BEARSSL = 
USE_GNUTLS = 
USE_HYPER = 
